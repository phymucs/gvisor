...
type dadState struct {
	...
	// Used to let a DAD timer know that it has been stopped while it is blocked
	// waiting to obtain the NIC's lock.
	//
	// Must only be read from or written to while protected by the lock of the NIC
	// this dadState is associated with.
	done *bool
}
...
func (ndp *ndpState) startDuplicateAddressDetection(addr tcpip.Address, ref *referencedNetworkEndpoint) *tcpip.Error {
	...
	if ref.getKind() != permanentTentative {
		// The endpoint should still be marked as tentative since we are starting
		// DAD on it.
	...
	timer = time.AfterFunc(0, func() {
		...
		if ndp.nic.stack.ndpDisp != nil {
			ndp.nic.stack.ndpDisp.OnDuplicateAddressDetectionStatus(ndp.nic.ID(), addr, dadDone, err)
		}
	})
...
func (ndp *ndpState) sendDADPacket(addr tcpip.Address) *tcpip.Error {
	...
	ref := ndp.nic.getRefOrCreateTemp(header.IPv6ProtocolNumber, header.IPv6Any, NeverPrimaryEndpoint, false, false, true)
